...
public class Mute extends Punishment {
    ...
    public Mute(String victim, String sender, String reason, Date punishDate, Date endingDate) {
        super(PunishmentType.MUTE, victim, sender, reason, punishDate, endingDate);
    }
    ...
    public void sendMuteMessage(ProxiedPlayer player) {
        if(player == null) {
            return;
        }
        SimpleDateFormat formatter = new SimpleDateFormat("dd.MM.yyyy:HH:mm:ss");
        String endDate = formatter.format(endingDate);
        player.sendMessage(Constant.PREFIX + "§7Du wurdest von HeroWars §4§lGemutet \n\n §7Grund: §4§l" + reason + "\n §7Ende des Mutes: §4§l" + endDate);
    }

    public void inform() {
        for(ProxiedPlayer player : ProxyServer.getInstance().getPlayers()) {
            if(player.hasPermission(Constant.MUTE_INFORM_PERMISSION)) {
                player.sendMessage(Constant.PREFIX + "§e" + ProxyServer.getInstance().getPlayer(UUID.fromString(sender)) + " §7hat den Spieler §c" + ProxyServer.getInstance().getPlayer(UUID.fromString(victim)) + " §7für §c" + reason + " §7gemutet.");
            }
        }
    }
